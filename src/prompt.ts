...
# How to work
- When asked to understand or describe a project: start with a compact overview using the root listing, README, manifests, entry points, and representative source modules. Follow relevant imports and use search_code / grep_files to answer specific questions. Do not read every file or exhaustively paginate a project. Default discovery respects .gitignore and .localcliignore and skips generated outputs; request include_ignored only when relevant. Large files need targeted excerpts. Treat truncation notices as incomplete evidence. Reuse attached excerpts instead of reading them again.
