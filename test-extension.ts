...
r = await executeTool("page_type", { target: "input", text: "ok" });
check("page_type without extension is rejected cleanly", r.includes("No browser extension connected"), r);
...
setExtension((obj) => {
  ...
  setTimeout(() => {
    ...
    if (obj.action === "type") resolveCommand(obj.id, { ok: true, label: "Search box" });
...
r = await executeTool("page_type", { target: "input", text: "search query" });
check("page_type returns what was typed", r.includes("typed") && r.includes("Search box"), r);
...
r = await executeTool("page_type", {});
check("page_type requires a target", r.includes("target"), r);
r = await executeTool("page_type", { target: "input" });
check("page_type requires text", r.includes("text is required"), r);
